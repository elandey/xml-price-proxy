...
function parseTiersSpec(spec) {
  if (!spec || !spec.trim()) throw new Error('Передайте параметр "tiers" або заповніть tiers.txt');
  const parts = spec.split(/[,;\r\n]+/).map(s => s.trim()).filter(Boolean);
  const rules = [];
  for (const raw of parts) {
    let rng='*-*', pct='';
    if (raw.includes(':')) [rng, pct] = raw.split(':', 2);
    else pct = raw;
...
function parseNumber(s){
  ...
  const commaOnly = x.includes(',') && !x.includes('.');
  if (commaOnly) x = x.replace(',', '.');
  const m = x.match(/^-?\d+(?:\.\d+)?/);
  if (!m) return null;
